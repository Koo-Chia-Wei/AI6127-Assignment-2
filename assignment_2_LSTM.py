...
class EncoderRNN(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.lstm = nn.LSTM(hidden_size, hidden_size)  # Replacing GRU with LSTM

    def forward(self, input, hidden):
        embedded = self.embedding(input).view(1, 1, -1)
        output, hidden = self.lstm(embedded, hidden)  # Use LSTM here
        return output, hidden

    def initHidden(self):
        return (torch.zeros(1, 1, self.hidden_size, device=device), 
                torch.zeros(1, 1, self.hidden_size, device=device))  # LSTM requires a tuple of (h_0, c_0)
...
class Decoder(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        output, hidden = self.lstm(output, hidden)  # Use LSTM here
