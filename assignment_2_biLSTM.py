...
class EncoderRNN(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        # Setting bidirectional=True makes the LSTM bidirectional.
        self.lstm = nn.LSTM(hidden_size, hidden_size, bidirectional=True)

    def forward(self, input, hidden):
        embedded = self.embedding(input).view(1, 1, -1)
        output, hidden = self.lstm(embedded, hidden)
        # Since the LSTM is bidirectional, we need to combine the outputs from
        # the forward and backward passes. You can sum, concatenate, or take the mean.
        output = output[:, :, :self.hidden_size] + output[:, :, self.hidden_size:]
        return output, hidden

    def initHidden(self):
        # Initializing hidden and cell states for both forward and backward directions.
        return (torch.zeros(2, 1, self.hidden_size, device=device),  # 2 for bidirection
                torch.zeros(2, 1, self.hidden_size, device=device))

class Decoder(nn.Module):
    def __init__(self, hidden_size, output_size, is_bidirectional_encoder=False):
        super(Decoder, self).__init__()
        self.hidden_size = hidden_size
        self.is_bidirectional_encoder = is_bidirectional_encoder
    ...
    def initHidden(self):
        multiplier = 2 if self.is_bidirectional_encoder else 1
        return (torch.zeros(multiplier, 1, self.hidden_size, device=device),
                torch.zeros(multiplier, 1, self.hidden_size, device=device))
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion, max_length=MAX_LENGTH):
    ...
    decoder_hidden = (torch.sum(encoder_hidden[0], dim=0, keepdim=True),
                      torch.sum(encoder_hidden[1], dim=0, keepdim=True))
